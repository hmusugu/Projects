...
removed_configs_file = "C:\\Users\\amasinha\\Documents\\Projects\\FCA\\IVS_Py27_Test\\US Only_04052017_Iteration2.7\\Removed_Configs_Retail_Only_04052017_Updated_for_10683_and_13652_Issue_MappingToProposed.csv"
#Input data CSV file - SIMS configuration data file
infile = "C:\\Users\\amasinha\\Documents\\Projects\\FCA\\IVS_Py27_Test\\US Only_04052017_Iteration2.7\\IVS_Input_Retail_Only_AddedProposedConfigs.csv"
#==============================================================================
# #Output data CSV file - transformed infile data necessary for distance calculation
# midfile = "C:\\Users\\tkress\\Desktop\\FCA\\IVS\\US Only_04052017_Iteration2.7\\KL_Data_transformed.csv"
#==============================================================================
#Output data CSV file - configuration IDs removed, corresponding configuration IDs
#transferred to, and amount of volume transferred
outfile_volume_transfer = "C:\\Users\\amasinha\\Documents\\Projects\\FCA\\IVS_Py27_Test\\US Only_04052017_Iteration2.7\\KL_Data_output_volume_transfer_MappingToProposed_Test_Aman_py27.csv"
#Output data CSV file - configuration IDs removed, contribution profit before transfer,
#and contribution profit after transfer
outfile_profit_change = "C:\\Users\\amasinha\\Documents\\Projects\\FCA\\IVS_Py27_Test\\US Only_04052017_Iteration2.7\\KL_Data_output_profit_change_MappingToProposed_Test_Aman_py27.csv"
#Output data CSV file - configuration IDs and their corresponding complexity scores,
#before and after volume substitution
outfile_complexity_score = "C:\\Users\\amasinha\\Documents\\Projects\\FCA\\IVS_Py27_Test\\US Only_04052017_Iteration2.7\\KL_Data_output_complexity_MappingToProposed_Test_Aman_py27.csv"
#Define the columns in infile that will be analyzed in the distance calculation, 
#including the prioritized column names (i.e. list of column names in data file that 
#correspond to options, and not contribution margin, variable marketing, etc.)
...
#Define the columns in option_columns that correspond to the features that can
#not change when a configuration is removed. E.g., consumers are not willing to
#switch into a configuration with a different series, engine type, etc:
#['VERSION_MFC','ENGINE_CAR_LT_TRK']
fixed_option_columns = []
#Define the option choices of specific option_columns that correspond to the features that can
#not change when a configuration is removed. E.g., consumers are not willing to
#switch out of a specific series to another series:
#['TRAILHAWK']
fixed_option_choices_values = ['TRAILHAWK']
#Define the columns in option_columns that correspond to the features that are
#prioritized (provide feature names, not column numbers) in terms of distance. E.g.:
#['DRIVE_CAR_LT_TRK','CAB_STYLE_LT_TRK','LIGHT_TRUCK_WHEELBASES','ENGINE_CAR_LT_TRK','GVWS']
prioritized_columns = []
#Define the prioritized_columns values that can only move into similar or higher features
#For instance, if there are 17", 18", and 19" Wheels, and removed configurations
#can only increase Wheel size, ranked_option_values = {'WHEELS': ['17" Wheels', '18" Wheels', '19" Wheels']}
...
#Provide the column name corresponding to Configuration IDs
...
#Provide the range of Net_Revenue that volume can substitute to (i.e. if a 
#removed configuration has net revenue of $30,000 and net_rev_range = 2000, 
#volume can only be transferred to configurations between $28,000 and $32,000)
net_rev_range = 2000
net_rev_percent = 0.05
...
#2.5% (before FixEngine)
# volume_buyback = {"SPORTAWD2.4L I4 MULTIAIR ENGINE": 2191.0, 
# "SPORTAWD2.4L I4 PZEV M-AIR ENGINE": 2191.0, 
# "TRAILHAWK4WD3.2L V6 24V VVT ENGINE W/ESS": 2622.0, 
# "TRAILHAWK4WD2.4L I4 MULTIAIR ENGINE": 4638.0, 
# "TRAILHAWK4WD2.4L I4 PZEV M-AIR ENGINE": 4638.0,
# "LATITUDEFWD2.4L I4 MULTIAIR ENGINE": 826.0,
# "LATITUDEFWD3.2L V6 24V VVT ENGINE W/ESS": 816.0,
# "LATITUDEFWD2.4L I4 PZEV M-AIR ENGINE": 826.0,
# "LIMITEDFWD2.4L I4 MULTIAIR ENGINE": 801.0}
...
#Checking inputs
if type(option_columns) != list or type(fixed_option_columns) != list or type(fixed_option_choices_values) != list or type(prioritized_columns) != list or type(ranked_option_values) != dict or type(config_ids_column) != str or type(contrib_prof_column) != str or type(net_rev_column) != str or type(volume_column) != str or type(total_parts_column) != str or type(unique_parts_column) != str or type(labor_time_column) != str or type(net_rev_range) != int or type(upper_bound_relaxation) != int:
...
if net_rev_range <= 0:
    print "Please define net_rev_range as an int greater than 0"
    sys.exit()

if upper_bound_relaxation < 0:
    print "Please define upper_bound_relaxation as an int greater than or equal to 0"
    sys.exit()
...
###############################################################################
#Start of Data Transformation of infile
###############################################################################

#Read data and format properly
myfile = open(infile,"rb")
reader = csv.reader(myfile,delimiter=',')
input_matrix = np.array(list(reader))
myfile.close()
...
#Determine column indices for each of the inputs
options_columns_indices = [list(input_matrix[0]).index(x) for x in option_columns]
fixed_option_columns_indices = [list(input_matrix[0]).index(x) for x in fixed_option_columns]
prioritized_columns_indices = [list(input_matrix[0]).index(x) for x in prioritized_columns]
...
#Create the new transformed matrices (one with all features, the other only with prioritized features)
...
#Calculate the complexity score for each of the configurations before substituting volume
min_labor_time = min([float(input_matrix[i][labor_time_column_index]) for i in range(1,len(input_matrix))])
max_labor_time = max([float(input_matrix[i][labor_time_column_index]) for i in range(1,len(input_matrix))])
min_total_part_count = min([float(input_matrix[i][total_parts_column_index]) for i in range(1,len(input_matrix))])
max_total_part_count = max([float(input_matrix[i][total_parts_column_index]) for i in range(1,len(input_matrix))])
min_unique_part_count = min([float(input_matrix[i][unique_parts_column_index]) for i in range(1,len(input_matrix))])
max_unique_part_count = max([float(input_matrix[i][unique_parts_column_index]) for i in range(1,len(input_matrix))])
complexity_score_before = []
for i in range(1,len(input_matrix)):
    current_volume = float(input_matrix[i][volume_column_index])
    current_labor_time = float(input_matrix[i][labor_time_column_index])
    current_total_parts = float(input_matrix[i][total_parts_column_index])
    current_unique_parts = float(input_matrix[i][unique_parts_column_index])
    ...
    #The complexity score calculation for the current analyzed configuration
    current_complexity_score = 0.40*volume_score + 0.20*(current_labor_time - min_labor_time)/(max_labor_time - min_labor_time) + 0.20*(current_total_parts - min_total_part_count)/(max_total_part_count - min_total_part_count) + 0.20*(current_unique_parts - min_unique_part_count)/(max_unique_part_count - min_unique_part_count)    
    complexity_score_before.append(current_complexity_score)

#==============================================================================
# #Output the data
# with open(midfile, 'wb') as csvfile:
#     writer = csv.writer(csvfile, delimiter=',',
#                             quotechar='"', quoting=csv.QUOTE_MINIMAL)
#     current_output = np.concatenate([[Config_IDs[0]], options_choices_unique])
#     writer.writerow(current_output)
# 
# with open(midfile, 'ab') as csvfile:
#     writer = csv.writer(csvfile, delimiter=',',
#                             quotechar='"', quoting=csv.QUOTE_MINIMAL)
#     for i in range(1,len(output_matrix)):
#         current_output = np.concatenate([[Config_IDs[i]], output_matrix[i]])        
#         writer.writerow(current_output)
# 
#==============================================================================
###############################################################################
#Start of Data Pull of removed_configs_file
###############################################################################

#Read data and format properly
myfile = open(removed_configs_file,"rb")
reader = csv.reader(myfile,delimiter=',')
removed_indices = list(reader)
myfile.close()

#Check that config_ids_column is consistent between removed_configs_file and infile
...
#Update input_matrix to recalculate the complexity score based on substituted volume
for i in volume_transfer_output:
    #Removed configuration has volume changed to 0
    input_matrix[Config_IDs.index(i[0])][volume_column_index] = 0
    #Configuration transferred to receives an updated volume
    if i[1] != 'LOST VOLUME':
        input_matrix[Config_IDs.index(i[1])][volume_column_index] = str(float(input_matrix[Config_IDs.index(i[1])][volume_column_index]) + float(i[2]))
    
#Calculate the complexity score for each of the configurations after substituting volume
min_labor_time = min([float(input_matrix[i][labor_time_column_index]) for i in range(1,len(input_matrix)) if i not in removed_indices])
max_labor_time = max([float(input_matrix[i][labor_time_column_index]) for i in range(1,len(input_matrix)) if i not in removed_indices])
min_total_part_count = min([float(input_matrix[i][total_parts_column_index]) for i in range(1,len(input_matrix)) if i not in removed_indices])
max_total_part_count = max([float(input_matrix[i][total_parts_column_index]) for i in range(1,len(input_matrix)) if i not in removed_indices])
min_unique_part_count = min([float(input_matrix[i][unique_parts_column_index]) for i in range(1,len(input_matrix)) if i not in removed_indices])
max_unique_part_count = max([float(input_matrix[i][unique_parts_column_index]) for i in range(1,len(input_matrix)) if i not in removed_indices])
...
for i in range(1,len(input_matrix)):
    if i not in removed_indices:
        ...
        #The complexity score calculation for the current analyzed configuration
        current_complexity_score = 0.40*volume_score + 0.20*(current_labor_time - min_labor_time)/(max_labor_time - min_labor_time) + 0.20*(current_total_parts - min_total_part_count)/(max_total_part_count - min_total_part_count) + 0.20*(current_unique_parts - min_unique_part_count)/(max_unique_part_count - min_unique_part_count)    
        complexity_score_after.append(current_complexity_score)
    else:
        complexity_score_after.append('Removed Configuration')
###############################################################################
#Output all of the Data
###############################################################################

#Volume transferred data
with open(outfile_volume_transfer, 'wb') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    current_output = np.concatenate([['Removed ' + config_ids_column],['Transfer to ' + config_ids_column],['Amount of transferred volume']])
    writer.writerow(current_output)

with open(outfile_volume_transfer, 'ab') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    for i in range(0,len(volume_transfer_output)):
        current_output = volume_transfer_output[i]     
        writer.writerow(current_output)

#Contribution profit change data
with open(outfile_profit_change, 'wb') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    current_output = np.concatenate([['Removed ' + config_ids_column],['Total contribution profit before transfer'],['Total contribution profit after transfer']])
    writer.writerow(current_output)

with open(outfile_profit_change, 'ab') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    for i in range(0,len(contrib_prof_transfer_output)):
        current_output = contrib_prof_transfer_output[i]     
        writer.writerow(current_output)

#Complexity score data
with open(outfile_complexity_score, 'wb') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    current_output = np.concatenate([[config_ids_column],['Complexity score before transfer'],['Complexity score after transfer']])
    writer.writerow(current_output)

with open(outfile_complexity_score, 'ab') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    for i in range(1,len(input_matrix)):
        current_output = [Config_IDs[i],complexity_score_before[i-1],complexity_score_after[i-1]]     
        writer.writerow(current_output)
        
# execfile('C:\Users\\tkress\Desktop\FCA\IVS\Volume_Substitution.py')